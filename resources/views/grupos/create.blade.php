...
<div class="container mt-4">
    ...
    <form action="{{ route('grupos.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="clave_grupo" id="clave_grupo" class="form-control" value="{{ old('clave_grupo') }}" maxlength="50" required>
        </div>

        <div class="mb-3">
            <label for="nombre_grupo" class="form-label">Nombre</label>
            <input type="text" name="nombre_grupo" id="nombre_grupo" class="form-control" value="{{ old('nombre_grupo') }}" maxlength="100" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="id_materia" id="id_materia" class="form-select" required>
                ...
                @foreach($materias as $id => $nombre)
                    <option value="{{ $id }}" {{ old('id_materia') == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="id_periodo" id="id_periodo" class="form-select" required>
                ...
                @foreach($periodos as $id => $nombre)
                    <option value="{{ $id }}" {{ old('id_periodo') == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="id_personal" id="id_personal" class="form-select" required>
                ...
                @foreach($personals as $id => $nombre)
                    <option value="{{ $id }}" {{ old('id_personal') == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="turno" id="turno" class="form-control" value="{{ old('turno') }}" maxlength="20">
        </div>
